...
class NotificationServiceTest extends IntegrationTestSupport {
  ...
  void createNotificationByInterest() {
    ...
    // when
    NotificationResult notificationByInterest = notificationService.createNotificationByInterest(
        savedUser.getId(), savedInterest.getId(), count);
  ...
  @Transactional
  @DisplayName("관심사 알림 생성시 해당 관심사를 구독한 유저가 없으면, 예외를 반환합니다.")
  @Test
  void createNotificationByInterest_UserNoException() {
    // given
    Interest savedInterest = interestRepository.save(new Interest("러닝머신"));
    long count = 0;

    // when & then
    Assertions.assertThatThrownBy(() -> notificationService.createNotificationByInterest(
            -1L, savedInterest.getId(), count))

...
public class UserActivityServiceImpl implements UserActivityService {
  ...
  public UserActivitiesResponse getUserActivities(Long userId) {
    ...
    List<SubscriptionActivityDto> subscriptions = userActivityRepository.findSubscriptionActivities(userId)
        .stream()
        .map(SubscriptionActivityDto::from)
        .toList();

    List<CommentActivityDto> comments = userActivityRepository.findCommentActivities(userId, ACTIVITY_LIMIT)
        .stream()
        .map(CommentActivityDto::from)
        .toList();

    List<CommentLikeActivityDto> likes = userActivityRepository.findCommentLikeActivities(userId, ACTIVITY_LIMIT)
        .stream()
        .map(CommentLikeActivityDto::from)
        .toList();

    List<ArticleViewActivityDto> views = userActivityRepository.findArticleViewActivities(userId, ACTIVITY_LIMIT)
        .stream()
